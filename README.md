...
- **Returns are driven by process, not product.** Two reason codes — System Input Error (SIS, 47.46%) and Customer Ordering Error (SPC, 33.90%) — account for 81% of all cases. Product category distribution is fairly even (8–16 cases each), which rules out a specific product or product line as the root cause.
